  ...
  async function rafraichir() {
    ...
    conteneur.innerHTML = `
      ...
      <div class="bloc-tableau">
        <table class="donnees">
          <thead><tr>${ongletActif === "A_ASSIGNER" ? "<th><input type=\"checkbox\" id=\"case-tout\"></th>" : "<th></th>"}<th>Ramassage</th><th>Commande</th><th>Expéditeur</th><th>Adresse</th><th>Hub</th><th>Livreur</th><th>Statut</th><th></th></tr></thead>
          <tbody>
            ${commandes.map((c) => `
              <tr>
                <td>${ongletActif === "A_ASSIGNER" ? `<input type="checkbox" class="case-commande" value="${c.id_commande}">` : ""}</td>
                <td class="cellule-donnee">${escapeHtml(c.id_ramassage || "—")}</td>
