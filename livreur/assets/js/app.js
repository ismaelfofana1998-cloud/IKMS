      ...
      <div class="carte-mission">
        ...
        <div class="carte-actions">
          ...
          ${restantAPayer.length
            ? `<button class="btn btn-primaire" data-action="payer-ramassage" data-commande="${idCommande}">Encaisser ${formaterFcfa(totalRestant)} FCFA (expéditeur)</button>`
            : `<button class="btn btn-primaire" data-action="ramasser" data-commande="${idCommande}">J'ai récupéré les colis</button>`}
        </div>
      </div>`;
  }).join("");
}
  ...
  return `
    <div class="carte-mission">
      <div class="carte-en-tete">
        <div>
          <div class="carte-nom">${escapeHtml(m.destinataire_nom)}</div>
          <div class="carte-id">${escapeHtml(m.id_commande)} · ${escapeHtml(m.id_colis)}${m.code_zone ? " · " + escapeHtml(m.code_zone) : ""}</div>
        </div>
        ${puce}
      </div>
      ${action ? `<div class="carte-actions">${action}</div>` : ""}
...
function carteLivraison(m) {
  ...
  return `
    <div class="carte-mission">
      ...
      <button class="btn btn-discret btn-pleine-largeur btn-echec-discret" data-action="echec" data-colis="${m.id_colis}">Signaler un échec de livraison</button>
    </div>`;
}
...
function carteRetour(m) {
  ...
  return `
    <div class="carte-mission">
      ...
      <div class="carte-actions">
        ${lien ? `<a class="lien-carte" href="${lien}" target="_blank" rel="noopener">${iconeMap()} Itinéraire</a>` : ""}
        <button class="btn btn-primaire" data-action="remise-expediteur" data-colis="${m.id_colis}">Valider la remise</button>
      </div>
    </div>`;
}
...
// Initiales pour l'avatar du topbar : premier caractère du prénom + premier
// caractère du dernier mot du nom ("Yao Bamba" -> "YB", "Fatou" -> "F").
...
// La cloche de notification et le statut "En ligne/Hors ligne" ont été
// retirés de l'interface (voir app.html) -- le statut ne servait à rien de
// concret pour le livreur, et la cloche reviendra avec l'application
// native. On garde quand même le comportement utile de l'événement "online"
// (relancer un rafraîchissement silencieux dès que la connexion revient).
...
(async function demarrer() {
  ...
  document.querySelector("#topbar-avatar").textContent = initiales(profil.nom);
  listerHubs().then((h) => {
    hubsDisponibles = h;
