...
export async function listerCommandesEnRamassage(idHubAgent = null) {
  ...
  let requete = sb()
    .from("commandes")
    .select("id_commande, id_ramassage, expediteur_nom, expediteur_tel, expediteur_adresse, code_ramassage, id_livreur_ramassage, cree_le, id_hub_prevu, alerte_zone_expediteur, hubs(nom)")
...
export async function listerCommandes({ idHubAgent = null, aujourdhuiSeulement = true, recherche = "", limite = 200 } = {}) {
  let requete = sb()
    .from("commandes")
    .select("id_commande, id_ramassage, expediteur_nom, expediteur_tel, code_ramassage, mode_paiement, id_livreur_ramassage, cree_le, id_hub_prevu, alerte_zone_expediteur, hubs(nom)")
    .order("cree_le", { ascending: false })
    .limit(limite);
  if (idHubAgent) requete = requete.eq("id_hub_prevu", idHubAgent);
  ...
  if (recherche.trim()) {
    ...
    requete = requete.or(`id_commande.ilike.%${q}%,id_ramassage.ilike.%${q}%,expediteur_nom.ilike.%${q}%,expediteur_tel.ilike.%${q}%`);
  }
  const { data, error } = await requete;
  if (error) throw error;
  return data || [];
